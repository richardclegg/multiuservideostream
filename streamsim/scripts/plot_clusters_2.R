...
clusters= subset(clusters,type !=3)   
shapelist<-c(19,2)
shapetypes<-c("1","2")
sizelist<-c(1.5,4)
...
world <- world +scale_shape_manual(values=shapelist,labels=c("user","centroid"))
world <- world + guides(colour="none",size="none", shape=
    guide_legend(title="node type",
    override.aes = list(colour=c("black","black"), size=sizelist,
    shape=shapelist)))
...
ggsave("graphs/cluster_plot_2.pdf",width=6.0,height=2.05)
#dev.off()
