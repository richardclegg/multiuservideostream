...
fname<-args[1]
outname<-args[2]
clusters<-read.table(fname,col.names=c("lat","lon","cluster","type"))                    
shapelist<-c(19,2,5)
shapetypes<-c("1","2","3")
sizelist<-c(1.5,4,5)
...
#Creat a base plot
p <- ggplot() + coord_fixed(ylim=c(-86,85),xlim=c(-180,180)) +theme_opts
...
world <- world +scale_colour_manual(values=c("#CC0000","#009900","#0000FF"))
...
ggsave(outname,width=6.0,height=2.05)
#dev.off()
