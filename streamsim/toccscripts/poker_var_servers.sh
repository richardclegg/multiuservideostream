...
OUT_FILE="toccdata/poker_var_ndynamic.out"
IN_FILE="toccscripts/poker_var_ndynamic.json"
...
OUT_FILE="toccdata/poker_var_nstatic.out"
IN_FILE="toccscripts/poker_var_nstatic.json"
rm -f $OUT_FILE
for servers in `seq 10`; do
    sed -e "s/XXXXXX/$servers/g" $IN_FILE | sed -e "s%DAYTMPFILE%$TMPDAYFILE%g" | sed -e "s%COSTTMPFILE%$TMPCOSTFILE%g" | sed -e "s%QOSTMPFILE%$TMPQOSFILE%g" | sed -e "s%SESSTMPFILE%$TMPSESSFILE%g"> $TMPJSONFILE
    src/streamsim.py $TMPJSONFILE
...
OUT_FILE="toccdata/poker_var_nrandom.out"
IN_FILE="toccscripts/poker_var_nrandom.json"
rm -f $OUT_FILE
for servers in `seq 10`; do
    sed -e "s/XXXXXX/$servers/g" $IN_FILE | sed -e "s%DAYTMPFILE%$TMPDAYFILE%g" | sed -e "s%COSTTMPFILE%$TMPCOSTFILE%g" | sed -e "s%QOSTMPFILE%$TMPQOSFILE%g" | sed -e "s%SESSTMPFILE%$TMPSESSFILE%g"> $TMPJSONFILE
    src/streamsim.py $TMPJSONFILE
