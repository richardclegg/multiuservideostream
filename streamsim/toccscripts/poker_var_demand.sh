...
OUT_FILE="data/poker_var_demand_ndynamic.out"
IN_FILE="scripts/poker_var_demand_ndynamic.json"
...
OUT_FILE="data/poker_var_demand_nrandom.out"
IN_FILE="scripts/poker_var_demand_nrandom.json"
rm -f $OUT_FILE
for demand in "1000.0" "2000.0" "3000.0" "4000.0" "5000.0"; do
    sed -e "s/XXXXXX/$demand/g" $IN_FILE | sed -e "s%DAYTMPFILE%$TMPDAYFILE%g" | sed -e "s%COSTTMPFILE%$TMPCOSTFILE%g" | sed -e "s%QOSTMPFILE%$TMPQOSFILE%g" | sed -e "s%SESSTMPFILE%$TMPSESSFILE%g"> $TMPJSONFILE
    src/streamsim.py $TMPJSONFILE
...
OUT_FILE="data/poker_var_demand_nstatic.out"
IN_FILE="scripts/poker_var_demand_nstatic.json"
rm -f $OUT_FILE
for demand in "1000.0" "2000.0" "3000.0" "4000.0" "5000.0"; do
    sed -e "s/XXXXXX/$demand/g" $IN_FILE | sed -e "s%DAYTMPFILE%$TMPDAYFILE%g" | sed -e "s%COSTTMPFILE%$TMPCOSTFILE%g" | sed -e "s%QOSTMPFILE%$TMPQOSFILE%g" | sed -e "s%SESSTMPFILE%$TMPSESSFILE%g"> $TMPJSONFILE
    src/streamsim.py $TMPJSONFILE
